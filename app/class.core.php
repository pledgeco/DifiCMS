...
class core implements iCore
{
	...
	{
		die('<b>' . $who . ' - DifiCMS: </b><br /> <center>' . $txt . '</center>');
	}
	...
	final public function handleCall($k)
	{
		...
		if($_CONFIG['difi']['in_maint'] == false)
		{
			if(!isset($_SESSION['user']['id']))
			{
				switch($k)
				{
					...
					case "realisering";
						...
						header('Location: '.$_CONFIG['difi']['url'].'/index');
						exit;
					break;
			...
			else
			{
				...
				{
					header('Location: '.$_CONFIG['difi']['url'].'/logout');
				}
		...
		{
			header('Location: '.$_CONFIG['difi']['url'].'/maintenance');
			exit;
		}
	}
	...
	final public function handleCallHK($k)
	{
		...
		if($_SESSION["in_hk"] != true)
		{
			if(isset($_SESSION['user']['id']))
			{
				...
				{
					header("Location:".$_CONFIG['difi']['url']."/ase/login");
					exit;
				}
			}
			else
			...
			else
			{
				if($k == 'balist')
				{
					...
					if(isset($_GET["unban"]))
					{
						...
						header("Location: ".$_CONFIG['difi']['url']."/ase/banlist");
						exit;
					}	
				}
			}
