...
class forms implements iForms
{
	...
	final public function getPageNews()
	{
		global $template, $engine;
			...
			{
				$_GET['id'] = mysql_result(mysql_query("SELECT MAX(id) FROM cms_news_old"), 0);
			}
				$result = mysql_query("SELECT title, id FROM cms_news_old WHERE id != '" . $engine->secure($_GET['id']) . "' ORDER BY id DESC");
				
				while($news1 = mysql_fetch_array($result))
				{
					$template->setParams('newsList', '&laquo; <a href="http://prosjektdatabasen.com/index.php?url=news&id='.$news1["id"].'">' . $news1['title'] . '</a><br/>');
				}
				
				$news = $engine->fetch_assoc("SELECT title, longstory, author, published FROM cms_news_old WHERE id = '" . $engine->secure($_GET['id']) . "' LIMIT 1");
				...
				$listQ = mysql_query("SELECT title, id FROM cms_news_old ORDER BY id DESC");
				$output = "";
	...
	final public function getPageHome()
	{
		global $template, $engine;
		$a = 1;
		$data = mysql_query("SELECT title, id, published, shortstory, image FROM cms_news_old ORDER BY id DESC LIMIT 5");
