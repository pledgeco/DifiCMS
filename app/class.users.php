...
class users implements iUsers
{
	...
	{
		if(strlen($username) <= 35 && preg_match('/^[a-z0-9 .\-]+$/i', $username)) 		
	 	{ 			
	 		return true; 		
	 	} 		 		
	...
	{
		global $engine;
		if($engine->num_rows("SELECT * FROM users WHERE ip_reg = '" . $_SERVER['REMOTE_ADDR'] . "'") == 9999999999999999999999)
		{
			return true;
		}
                                                                        ...
                                                                        if($this->isBanned($_SERVER['REMOTE_ADDR']) == false)
                                                                        {
                                                                                if(!$this->hasClones($_SERVER['REMOTE_ADDR']))
                                                                                {
                                                                                    if (@$_POST['captcha'] == $_SESSION['security_number'])
                                                                                    {
                                                                                        ...
                                                                                        if(!isset($template->form->reg_figure)) { $template->form->reg_figure = $_CONFIG['difi']['figure']; }
                                                                               
                                                                                        $this->addUser($template->form->reg_username, $core->hashed($template->form->reg_password), $template->form->reg_email, $template->form->reg_motto, $_CONFIG['difi']['credits'], $_CONFIG['difi']['pixels'], 2, $template->form->reg_figure, $template->form->reg_gender, $core->hashed($template->form->reg_key));
                                                                                        ...
                                                                                        header('Location: ' . $_CONFIG['difi']['url'] . '/me');
                                                                                        exit;
                                                                                    }
                                                                                    else
                                                                                    {
			...
			if(isset($template->form->username) && isset($template->form->password))
			{
				...
				{	 
					if($this->userValidation($template->form->username, $core->hashed($template->form->password)))
					{
						...
						{
							$_SESSION["in_hk"] = true;
							header("Location:".$_CONFIG['difi']['url']."/ase/dash");
							exit;
						}
						else
						{
	...
	final public function help()
	{
		...
		{
			$to = $_CONFIG['difi']['email'];
 			$subject = "Help from DifiCMS user - " . $this->getInfo($_SESSION['user']['id'], 'username');
	...
	final public function updateAccount()
	{
		...
		if(isset($_POST['account']))
		{
			...
			if(isset($_POST['acc_motto']) && strlen($_POST['acc_motto']) < 30 && $_POST['acc_motto'] != $this->getInfo($_SESSION['user']['id'], 'motto'))
			{
				...
				header('Location: '.$_CONFIG['difi']['url'].'/account');
				exit;
			}
			else
			{
